...
router.post('/', validateBody(createAdSlotInputSchema), async (req: Request, res: Response) => {
  try {
...
router.put('/:id', validateBody(updateAdSlotInputSchema), async (req: Request, res: Response) => {
  try {
    const { user } = req as AuthRequest;
    const id = getParam(req.params.id);
