...
import { createSponsorInputSchema, updateSponsorInputSchema } from '@anvara/schemas';
...
router.put('/:id', validateBody(updateSponsorInputSchema), async (req: Request, res: Response) => {
  try {
    const { user } = req as AuthRequest;
    const id = getParam(req.params.id);
